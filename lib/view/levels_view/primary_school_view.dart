...
// Add this method to dispose the stream controller when no longer needed
  ...
  /* Future<void> filterTutorsByName(String tutorName,
      QuerySnapshot<Map<String, dynamic>> filteredTutors) async {
    try {
      print('TutorName: $tutorName');

      // Filter tutors by name if tutorName is not empty
      if (tutorName.isNotEmpty) {
        String searchTerm = tutorName.toLowerCase();

        List<DocumentSnapshot<Map<String, dynamic>>> filteredTutorList = [];

        // Iterate over each document in the filteredTutors snapshot
        for (var tutorDoc in filteredTutors.docs) {
          String tutorNameInFirestore =
              (tutorDoc.data()?['name'] ?? '').toLowerCase();
          if (tutorNameInFirestore.contains(searchTerm.toLowerCase())) {
            // Add the tutor to the filtered list if the name matches the search term
            filteredTutorList.add(tutorDoc);

            // Display the tutor
            print(tutorDoc
                .data()); // You can replace this with any UI display logic
          }
        }

        // Update the UI with the filtered list of tutors
        setState(() {
          _tutorsStreamController.add(filteredTutorList);
          _tutorsStream = _tutorsStreamController.stream;
        });
      }
    } catch (error) {
      print('Error filtering tutors by name: $error');
      // Handle errors
    }
  } */
}
 ...
 /*Future<void> filterTutors(FilterOptions filterOptions) async {
    try {
      print('selectedYears: ${filterOptions.selectedDays}');
      // Fetch all tutors
      Query<Map<String, dynamic>> query = FirebaseFirestore.instance
          .collection('levels')
          .doc('primary')
      ...
      if (filterOptions.selectedDays != null &&
          filterOptions.selectedDays!.isNotEmpty) {
        for (var day in filterOptions.selectedDays!) {
          query = query.where('teachingSchedule.$day', isEqualTo: true);
        }
      }
      ...
      // Print tutors after filtering
      print('Filtered Tutors:');
      for (var tutorDoc in filteredSnapshot.docs) {
        print(tutorDoc.data()); // Print tutor data
      }
